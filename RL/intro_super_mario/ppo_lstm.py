...
class PPOBuffer:
    ...
    def store(self, obs, act, rew, val, logp, hidden_h, hidden_c):
        ...
        self.ptr += 1
    #完成一次episode后使用GAE-Lambda计算优势函数和下一状态的价值
...
def ppo(env_fn, actor=nn.Module, critic=nn.Module, ac_kwargs=dict(), seed=0,
    ...
    #如果有预训练,就加载预训练模型,没有就初始化actor网络和critic网络
    if pretrain != None:
   ...
   # sync_params(ac_v)
    ...
    # Set up function for computing PPO policy loss,计算策略损失函数
    ...
    # Set up function for computing value loss,计算值网络的损失函数
